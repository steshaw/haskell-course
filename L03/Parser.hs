...
{-data Status = Ok Int | Fail

data Parser a = P (Input-> Validation (Input, a))
parse :: Parser a -> (Input -> Validation (Input, a))
parse (P f) = f
-}
...
bindParser p f = P (\i -> case parse p i of
    Error e -> Error e
